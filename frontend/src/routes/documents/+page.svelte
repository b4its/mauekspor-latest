	...
	<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
		<Card>
			<CardContent class="p-5">
				<span class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Total documents</span>
				<strong class="mt-2 block text-3xl font-bold tracking-tight">{tradeDocuments.items.length}</strong>
			</CardContent>
		</Card>
		<Card>
			<CardContent class="p-5">
				<span class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Need attention</span>
				<strong class="mt-2 block text-3xl font-bold tracking-tight">{needsReviewCount}</strong>
			</CardContent>
		</Card>
		<Card>
			<CardContent class="p-5">
				<span class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Validation score</span>
				...
				<a href={`/documents/${document.id}`} class="block h-full p-5 no-underline">
					...
					<div class="mt-4 grid grid-cols-2 gap-2">
						<div class="rounded-lg border bg-muted/40 p-3 text-xs font-bold text-muted-foreground">
							ID <strong class="mt-1 block text-sm font-bold text-foreground">{document.id}</strong>
						</div>
						<div class="rounded-lg border bg-muted/40 p-3 text-xs font-bold text-muted-foreground">
							Version <strong class="mt-1 block text-sm font-bold text-foreground">{document.version}</strong>
						</div>
						<div class="rounded-lg border bg-muted/40 p-3 text-xs font-bold text-muted-foreground">
							Owner <strong class="mt-1 block text-sm font-bold text-foreground">{document.owner}</strong>
						</div>
						<div class="rounded-lg border bg-muted/40 p-3 text-xs font-bold text-muted-foreground">
							Updated <strong class="mt-1 block text-sm font-bold text-foreground">{document.updatedAt}</strong>
						</div>
					</div>
				</a>
			</Card>
		{:else}
			<div class="rounded-xl border border-dashed p-6 text-center font-semibold text-muted-foreground">No document matched your search.</div>
		{/each}
	</div>
</AppShell>