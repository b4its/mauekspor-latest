<script lang="ts">
	...
	async function handleVerify() {
		...
		try {
			...
			error = 'Gagal memverifikasi supplier.';
		}
	}
...
<AppShell title={data.supplier.name} eyebrow="Supplier detail">
	<Card class="bg-gradient-to-br from-background to-secondary/40 shadow-sm p-6 md:p-8">
		<div class="flex flex-wrap items-end justify-between gap-6">
			...
			<div class="shrink-0 rounded-xl border bg-muted/30 px-5 py-4 text-right">
				<span class="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Capability</span>
	...
	<div class="grid gap-4 md:grid-cols-2">
		<Card class="md:col-span-2">
			<CardHeader class="flex-row flex-wrap items-start justify-between gap-3">
				<div>
					...
					<CardDescription>Supplier readiness for export matching, costing, production planning, and compliance evidence collection.</CardDescription>
				</div>
				<div class="flex flex-wrap gap-2.5">
					<Button variant="outline" onclick={handleRequestEvidence}>{evidenceRequested ? 'Evidence requested' : 'Request evidence'}</Button>
					<Button onclick={handleVerify}>{verified ? 'Verified' : 'Verify supplier'}</Button>
				</div>
				{#if error}
			...
			<CardContent class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
				<div class="rounded-lg border bg-muted/40 p-3 text-xs font-bold text-muted-foreground">
					Capacity <strong class="mt-1 block text-sm font-bold text-foreground">{data.supplier.capacity}</strong>
				</div>
				<div class="rounded-lg border bg-muted/40 p-3 text-xs font-bold text-muted-foreground">
					Lead time <strong class="mt-1 block text-sm font-bold text-foreground">{data.supplier.leadTime}</strong>
				</div>
				<div class="rounded-lg border bg-muted/40 p-3 text-xs font-bold text-muted-foreground">
					Quality score <strong class="mt-1 block text-sm font-bold text-foreground">{data.supplier.qualityScore}%</strong>
				</div>
				<div class="rounded-lg border bg-muted/40 p-3 text-xs font-bold text-muted-foreground">
					Compliance score <strong class="mt-1 block text-sm font-bold text-foreground">{data.supplier.complianceScore}%</strong>
				</div>
				<div class="rounded-lg border bg-muted/40 p-3 text-xs font-bold text-muted-foreground">
					Next audit <strong class="mt-1 block text-sm font-bold text-foreground">{data.supplier.nextAudit}</strong>
				</div>
				<div class="rounded-lg border bg-muted/40 p-3 text-xs font-bold text-muted-foreground">
					Contact <strong class="mt-1 block text-sm font-bold text-foreground">{data.supplier.contact}</strong>
				</div>
			</CardContent>
		</Card>
		...
		<Card class="md:col-span-2 bg-gradient-to-br from-primary/10 to-background">
			<CardHeader class="p-0">
				<Badge variant="secondary">Linked products</Badge>
				<CardTitle>Product Capability</CardTitle>
			</CardHeader>
				...
				<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
					{#each data.linkedProducts as product}
						<a href={`/products/${product.id}`} class="grid gap-1.5 rounded-lg border bg-muted/30 p-3.5 no-underline transition-colors hover:border-ring/40">
							...
							<small class="text-sm text-muted-foreground">{product.packaging} · {product.readiness}% ready</small>
						</a>
					{/each}
				</div>
				{#if evidenceRequested}
					<p class="rounded-lg bg-primary/10 px-3 py-2 text-sm font-bold text-primary">Permintaan bukti dikirim ke backend.</p>
				{/if}
				{#if verified}
					<p class="rounded-lg bg-primary/10 px-3 py-2 text-sm font-bold text-primary">Supplier diverifikasi di backend.</p>
				{/if}
			</CardContent>
		</Card>
	</div>
